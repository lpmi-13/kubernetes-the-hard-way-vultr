...
EOF
  ...
  instance_id=$(vultr-cli instance list | grep worker-${i} \
    | awk -F ' ' '{print $1}')
  external_ip=$(vultr-cli instance list | grep worker-${i} \
    | awk -F ' ' '{print $2}')
  internal_ip=$(vultr-cli instance get $instance_id | grep -i "internal ip" | awk -F ' ' '{print $3}')
